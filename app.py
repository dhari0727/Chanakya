...
class SearchEngine:
    ...
    def _get_enhanced_contextual_results(self, query, num_results):
        ...
        if any(word in query_lower for word in ['weather', 'temperature', 'forecast']):
            results.extend(self._generate_weather_results(query))
        elif any(word in query_lower for word in ['news', 'breaking', 'latest']):
            results.extend(self._generate_news_results(query))
        elif any(char in query for char in '+-*/=') and any(c.isdigit() for c in query):
            calc_results = self._handle_calculations(query)
            if calc_results:
                results.extend(calc_results)
        
        # Technology-related queries
        tech_keywords = ['python', 'javascript', 'react', 'flask', 'programming', 'code', 'development']
        ...
        # Load and filter mock results if we need more
        if len(results) < num_results:
            try:
                ...
                for result in mock_results:
                    ...
                    snippet_matches = sum(1 for word in query_words if any(word in snippet_word for snippet_word in snippet_words))
                    
                    if title_matches > 0 or snippet_matches > 0:
                        results.append(result)
                
                # If still not enough results, add some general ones
                if len(results) < num_results:
                    remaining_needed = num_results - len(results)
                    for result in mock_results:
                        if len(results) >= num_results:
                            break
